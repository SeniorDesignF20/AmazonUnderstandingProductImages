...
from colorrandomization import singleChannel
...
def test_cutandpaste(size='small'):
	...
	for folder in os.listdir(test_images_path):
		...
		image_color = singleChannel(np.array(Image.open(image)))

		concatenator = Concatenator(csvfile=None, image_dim=image_dim)
		input_tensor = concatenator.concatenate2(image, image_color)
		...
		second_image = concatenator.transform_image2(image_color).numpy()
